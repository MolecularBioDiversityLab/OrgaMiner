    ...
    while retries < max_retries:
        try:
            if not os.path.exists(f"references/{genus}.fa"):
                query = f'{genus}[Organism] AND mitochondrion[filter] AND 8000:60000[Sequence Length]'
...
def pt_ref_finder(genus, logging):
    ...
    while retries < max_retries:
        try:
            if not os.path.exists(f"references/{genus}.fa"):
                query = f'{genus}[Organism] AND chloroplast[filter] AND 80000:600000[Sequence Length]'
